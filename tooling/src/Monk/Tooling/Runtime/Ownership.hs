...
runPortable ctx = do
  ...
  withWorkspace $ \directory -> do
    ...
    void $ evaluate (B.length remainingOut + B.length remainingErr)
...
runNativeLauncher ctx = do
  ...
  withWorkspace $ \directory -> do
    ...
    void $ evaluate (B.length outputBytes + B.length errorBytes)
    ended <- waitForProcess process
    ...
    void $ evaluate (B.length rest + B.length restErr)
    closeFd release
...
runSignals ctx = do
  ...
  forM_ (["external", "pipeline", "body", "substitution"] :: [String]) $ \kind ->
    forM_ [False, True] $ \inherited -> withWorkspace $ \directory -> do
      ...
      out <- B.hGetContents output
      err <- B.hGetContents errors
      void $ evaluate (B.length out + B.length err)
      ended <- waitForProcess process
