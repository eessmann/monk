...
tests = do
  ...
  assert "removal keeps quoted star literal beside active question" (trimPatternParts True False "a*bc" [(False, "a*"), (True, "?")] == "c")
  assert "longest suffix removal respects a quoted question" (trimPatternParts False True "xx?x?" [(False, "?"), (True, "*")] == "xx")
  assert "legacy active removal remains shortest" (trimPattern True False "abab" "a*" == "bab")
