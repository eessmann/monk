...
"$runtime" --abi 2 child-capture 3</dev/null < "$work/input" > "$work/actual" 2> "$work/error"
...
set +e
"$runtime" --abi 2 child-run 3< "$work/stdin" < "$work/input" > "$work/actual"
status=$?
set -e
[[ $status == 9 ]]
printf %s 'original stdin' > "$work/expected"
cmp "$work/expected" "$work/actual"
printf 'malformed' | "$runtime" --abi 2 child-capture 3</dev/null > "$work/actual"
...
printf '%s\0' '' 7 'test ! -e /dev/fd/9' 2 > "$work/input"
"$runtime" --abi 2 child-run 9< "$work/input" 3</dev/null < "$work/input"
...
printf '%s\0' '' 5 "command bash -c 'test ! -e /dev/fd/1'" 2 > "$work/input"
"$runtime" --abi 2 child-run 3</dev/null < "$work/input" 1>&-
printf '%s\0' '' 3 "command bash -c 'test ! -e /dev/fd/2'" 2 > "$work/input"
"$runtime" --abi 2 child-run 3</dev/null < "$work/input" 2>&-
# A real external producer must terminate with SIGPIPE, not loop or become 125.
printf '%s\0' '' 7 'command yes' 2 > "$work/input"
set +e
"$runtime" --abi 2 child-run 3</dev/null < "$work/input" | head -c 1 > "$work/actual"
status=${PIPESTATUS[0]}
set -e
[[ $status == 141 ]]
...
set +e
"$runtime" --abi 2 child-run 3</dev/null < "$work/input" > "$work/actual"
status=$?
set -e
[[ $status == 7 ]]
...
set +e
"$runtime" --abi 2 descriptor-state
all_open=$?
"$runtime" --abi 2 descriptor-state 0<&-
closed_in=$?
"$runtime" --abi 2 descriptor-state 1>&-
closed_out=$?
"$runtime" --abi 2 descriptor-state 2>&-
closed_err=$?
"$runtime" --abi 2 descriptor-state 0<&- 1>&- 2>&-
all_closed=$?
set -e
...
set +e
"$runtime" --abi 2 child-run 3<&- < "$work/input" 2> "$work/error"
status=$?
set -e
[[ $status == 125 ]]
