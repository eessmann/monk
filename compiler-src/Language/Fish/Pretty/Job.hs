...
prettyJobPipelineWith prettyStmt prettyExpr (MkFishJobPipeline time vars stmt conts) =
  let timeDoc = if time then "command time" <> space else mempty
        ...
        map
          ( \continuation ->
              let v = jpcVariables continuation
                  s = stageToStatement (jpcStatement continuation)
   ...
   in group (headDoc <> pipesDoc)

prettyVarAssign :: (forall t. FishExpr t -> Doc ann) -> VariableAssignment -> Doc ann
prettyVarAssign prettyExpr (MkVariableAssignment name mval) =
  case mval of
    Nothing -> pretty (identifierText name) <> "="
    Just v -> pretty (identifierText name) <> "=" <> prettyExpr v
...
prettyJobConjunctionWith ::
  ...
  FishJobConjunction ->
  Doc ann
prettyJobConjunctionWith prettyStmt prettyExpr (MkFishJobConjunction mdec job conts) =
