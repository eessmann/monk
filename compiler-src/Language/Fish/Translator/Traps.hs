...
exitWithStatusAt prefix origin status = Stmt (Command (compilerCommandName (prefix <> "trap_exit_run")) [ExprVal status, ExprVal origin, ExprVal (ExprVariable (VarAll "argv"))])

errorHook :: Text -> [ExprOrRedirect] -> FishStatement
errorHook prefix location = Stmt (Command (compilerCommandName (prefix <> "trap_err_run")) (location <> [ExprVal (ExprVariable (VarAll "argv"))]))
...
definitions prefix = [function "trap_exit_run" exitBody, function "trap_err_run" errorBody]
  where
    ...
    invoke role = Stmt (CommandExpr (variableExecutable (compilerIdentifier (name role))) [ExprVal (ExprVariable (VarAll "argv"))])
    ...
    exitBody =
      [ set [SetLocal] (name "trap_exit_status") (ExprQuotedVariable (VarIndex "argv" (IndexSingle (ExprNumLiteral 1)))),
        ...
        choose
          ...
          [],
        choose (equal "pending_signal" "0") [] [Session.request prefix Request.FinishBrokenPipe],
    ...
    errorBody =
      [ set [SetLocal] (name "trap_err_status") (variable sourceStatus),
        ...
        choose
          ...
          [],
        Stmt (ReturnScalar (variable (name "trap_err_status")))
      ]
