...
setList flags name values = Stmt (Decorated DecBuiltin (Set (SetUnpath : flags) (compilerIdentifier name) values))

conditional :: FishCommand grammar TStatus -> [FishStatement] -> [FishStatement] -> FishStatement
conditional condition yes no =
  let jobs = MkFishJobList (MkFishJobConjunction Nothing (MkFishJobPipeline False [] (Stmt condition) []) [] NE.:| [])
