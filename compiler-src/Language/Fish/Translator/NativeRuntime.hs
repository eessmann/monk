...
nativeRuntimeDefinition prefix bindings =
  ...
  where
    shadows = [Stmt (Decorated DecBuiltin (Set [SetLocal, SetUnexport, SetUnpath] (compilerIdentifier name) (ExprVariable (VarAll (compilerIdentifier name))))) | name <- S.toAscList (S.delete "PWD" bindings)]
    invoke = Stmt (Decorated DecCommand (CommandExpr (variableExecutable (compilerIdentifier (runtimePathName prefix))) [arg (ExprVariable (VarAll "argv"))]))
...
nativeWriterDefinition prefix =
  Stmt (Function (MkFishFunction (nativeWriterName prefix) [FuncCaptureVariable (compilerIdentifier pathName)] [] (producer :| [save, terminate, Stmt (ReturnScalar (scalar resultName))])))
  where
    pathName = runtimePathName prefix
    resultName = prefix <> "writer_status"
    producer = Stmt (Pipeline (MkFishJobPipeline False [] (builtin "printf" [lit "%s\\0", arg (ExprVariable (VarAll "argv"))]) [PipeTo [] (Stmt (Decorated DecCommand (CommandExpr (variableExecutable (compilerIdentifier pathName)) [lit "--abi", lit "2", lit "write-builtin"])))]))
    save = Stmt (Decorated DecBuiltin (Set [SetLocal] (compilerIdentifier resultName) (ExprListLiteral [scalar "status"])))
    terminate = choose (builtin "test" [arg (scalar resultName), lit "=", lit "141"]) [Stmt (Exec (variableExecutable (compilerIdentifier pathName)) [lit "--abi", lit "2", lit "raise-signal", lit "13"])] []
...
nativeWriterInvocation prefix range name arguments =
  let origin = maybe "<input>" (srcFile . rangeStart) range
   ...
   in Stmt (Command (compilerCommandName (nativeWriterName prefix)) (map lit [origin, line, name] <> arguments))
...
nativeRuntimeSetup cfg prefix requiredOperations
  ...
  | otherwise =
      [ set "fish_read_limit" (ExprLiteral "0"),
        ...
        choose (builtin "test" [lit "-n", arg (scalar pathName)]) [] [Stmt (Decorated DecBuiltin (Set [] (compilerIdentifier pathName) (ExprListLiteral [selected])))],
        Stmt (Decorated DecBuiltin (Set [SetLocal] (compilerIdentifier pathName) resolvedPath)),
        Stmt (Decorated DecBuiltin (Set [SetLocal] (compilerIdentifier description) (ExprCommandSubst (describe :| [])))),
        ...
        Stmt (Decorated DecBuiltin (Set [SetLocal] (compilerIdentifier capabilities) (ExprCommandSubst (builtin "string" [lit "split", lit " ", lit "--", arg (member description 2)] :| []))))
      ]
        <> [choose (builtin "contains" [lit "--", lit (nativeOperationName operation), arg (ExprVariable (VarAll (compilerIdentifier capabilities)))]) [] failure | operation <- S.toAscList operations]
        <> [choose (Stmt (Decorated DecCommand (CommandExpr (variableExecutable (compilerIdentifier pathName)) [lit "--abi", lit "2", lit "pipe-paths", RedirectVal (FileRedirect 1 OverwriteFile (ExprLiteral "/dev/null")), RedirectVal (FileRedirect 2 OverwriteFile (ExprLiteral "/dev/null"))]))) [] failure | S.member NativePipePaths operations]
  where
    ...
    resolvedPath = ExprCommandSubst (Stmt (Pipeline (MkFishJobPipeline False [] (builtin "path" [lit "resolve", lit "--null-out", lit "--", arg (scalar pathName)]) [PipeTo [] (builtin "string" [lit "split0"])])) :| [])
    selected = case translationRuntime cfg of
      RuntimeOnPath -> capture (Stmt (CommandSearch "monk-runtime"))
    ...
    describe = Stmt (Decorated DecCommand (CommandExpr (variableExecutable (compilerIdentifier pathName)) [lit "--describe", RedirectVal (FileRedirect 0 InputFile (ExprLiteral "/dev/null")), RedirectVal (FileRedirect 2 OverwriteFile (ExprLiteral "/dev/null"))]))
    failure =
      [ builtin "printf" [lit "%s\n", lit "monk.runtime: missing or incompatible native runtime", RedirectVal (DuplicateRedirect 1 WriteTo 2)],
        if entryMode cfg == Sourceable then Stmt (ReturnScalar (ExprLiteral "125")) else builtin "exit" [lit "125"]
      ]
...
entryDirectory prefix = ExprQuotedCommandSubst (filename :| [original, directory, result])
  where
    ...
    original = choose (builtin "test" [arg (scalar fileName), lit "=", arg (scalar "MONK_LAUNCH_WRAPPER"), lit "-a", lit "-n", arg (scalar "MONK_LAUNCH_ORIGINAL")]) [Stmt (Decorated DecBuiltin (Set [] (compilerIdentifier fileName) (ExprListLiteral [scalar "MONK_LAUNCH_ORIGINAL"])))] []
    directory = Stmt (Decorated DecBuiltin (Set [SetLocal] (compilerIdentifier directoryName) (ExprCommandSubst (Stmt (Pipeline (MkFishJobPipeline False [] (builtin "path" [lit "dirname", lit "--null-out", lit "--", arg (scalar fileName)]) [PipeTo [] (builtin "string" [lit "split0"])])) :| []))))
...
set name value = Stmt (Decorated DecBuiltin (Set [SetLocal] (compilerIdentifier name) (ExprListLiteral [value])))
...
choose predicate yes no =
  Stmt (If (MkFishJobList (MkFishJobConjunction Nothing (MkFishJobPipeline False [] predicate []) [] :| [])) (fromMaybe (builtin "true" [] :| []) (nonEmpty yes)) no [])
