...
import Monk.Runtime.Abi2 (BodyOpcode (BodySnapshot))
...
data ChildInvocation = MkChildInvocation
  { childPrelude :: [FishStatement],
    ...
    childSessionStage :: Request.Stage BodySnapshot,
...
materializeChild prefix mode runtime bindings arrays definitions body = do
  ...
  let snapshots = zipWith snapshot [0 :: Int ..] names
      ...
      loadState = ExprCommandSubst (builtin "string" [value (ExprLiteral "split0"), RedirectVal (FileRedirect 0 InputFile (argument 1))] NE.:| [])
      script =
        MkScript
          ( [set [SetGlobal] "fish_read_limit" (ExprLiteral "0"), setList [SetGlobal] "argv" loadState]
              ...
              <> [ Stmt (Function (MkFishFunction wrapper [FuncUnknownFlag "--no-scope-shadowing"] [] (bodyNE (body <> [Stmt (ReturnScalar finalStatus)])))),
                   Stmt (Command (compilerCommandName wrapper) [value (ExprVariable (VarAll "argv"))]),
      ...
      stage = Request.SnapshotStage script (variable "SHLVL") (arguments <> [SomeArgument (ListArgument (ExprVariable (VarAll "argv")))])
      frames = map argumentExpression (NE.tail (Request.stageArguments stage))
      shadows = [setList [SetLocal, SetUnexport, SetUnpath] name (ExprVariable (VarAll (compilerIdentifier name))) | name <- names <> S.toAscList arrays]
  ...
  where
    snapshot index name =
      let present = prefix <> "_present_" <> show index
       ...
       in (setup, map (SomeArgument . ScalarArgument . variable) [present, exported, count, saved], restore)
      ...
      let present = prefix <> "_array_present_" <> show index
          ...
          setup =
            [ set [SetLocal] present (ExprLiteral "0"),
              ...
              setList [SetLocal] saved (ExprVariable (VarAll (compilerIdentifier name))),
              set [SetLocal] count (ExprQuotedCommandSubst (builtin "count" [value (ExprVariable (VarAll (compilerIdentifier saved)))] NE.:| []))
            ]
       ...
       in (setup, [SomeArgument (ScalarArgument (variable present)), SomeArgument (ScalarArgument (variable count)), SomeArgument (ListArgument (ExprVariable (VarAll (compilerIdentifier saved))))], restore)
...
invocationFrames :: ChildInvocation -> [ExprOrRedirect]
invocationFrames = map argumentExpression . NE.tail . Request.stageArguments . childSessionStage
...
materializeCapture prefix range invocation = do
  ...
  let packet = prefix <> "_packet"
      ...
      capture = ExprCommandSubst (Stmt (Pipeline (MkFishJobPipeline False [] producer [PipeTo [] (builtin "string" [value (ExprLiteral "split0")])])) NE.:| [])
      member index = ExprQuotedVariable (VarIndex (compilerIdentifier packet) (IndexSingle (ExprNumLiteral index)))
      validPacket = builtinCommand "test" [value (ExprCommandSubst (builtin "count" [value (ExprVariable (VarAll (compilerIdentifier packet)))] NE.:| [])), value (ExprLiteral "="), value (ExprLiteral "3")]
...
materializePipeline prefix pipefail invocations =
  let statuses = prefix <> "_statuses"
      ...
      pipeline = Stmt (Pipeline (MkFishJobPipeline False [] (childCommand firstInvocation) (map (PipeTo [] . childCommand) remaining)))
      choose = Stmt (For (compilerIdentifier current) (ExprVariable (VarAll (compilerIdentifier statuses))) (conditional (equals (variable current) "0") [] [set [] selected (variable current)] NE.:| []) [])
      statements =
        concatMap childPrelude (toList invocations)
          <> [ pipeline,
               setList [SetLocal] statuses (ExprVariable (VarAll "pipestatus")),
               set [SetLocal] selected (ExprQuotedVariable (VarIndex (compilerIdentifier statuses) (IndexSingle (ExprNumLiteral (-1))))),
...
builtinCommand :: Text -> [ExprOrRedirect] -> FishCommand Atomic TStatus
builtinCommand name = Decorated DecBuiltin . Command (compilerCommandName name)
...
setList flags name values = Stmt (Decorated DecBuiltin (Set flags (compilerIdentifier name) values))

equals :: FishExpr TStr -> Text -> FishCommand Atomic TStatus
...
conditional :: FishCommand grammar TStatus -> [FishStatement] -> [FishStatement] -> FishStatement
conditional condition yes no =
  Stmt (If (MkFishJobList (MkFishJobConjunction Nothing (MkFishJobPipeline False [] (Stmt condition) []) [] NE.:| [])) (bodyNE yes) no [])
...
transportCommand runtimeName mode warning frames shadows =
  let descriptorMask = runtimeName <> "_child_descriptors"
      ...
      driver = Stmt (Command (compilerCommandName runtimeName) (map (value . ExprLiteral) ["--abi", "2", mode]))
      pipe = Stmt (Pipeline (MkFishJobPipeline False [] writer [PipeTo [] driver]))
      withInput = Stmt (Begin (pipe NE.:| []) [DuplicateRedirect 3 ReadFrom 0])
      ...
      probe = Stmt (Command (compilerCommandName runtimeName) (map (value . ExprLiteral) ["--abi", "2", "descriptor-state"]))
