...
import Monk.Runtime.Abi2 (CliOpcode (CliInteger), cliOpcodeName)
...
materializeArithmetic nativeHelper prefix getter setter expression
  ...
  where
    ...
    operation :: Maybe ArithmeticOrigin -> Primitive.Primitive CliInteger -> State.State Int ([FishStatement], FishExpr TStr)
    operation origin request = do
      valueName <- fresh
      packetName <- fresh
      let packet = ExprVariable (VarAll (compilerIdentifier packetName))
          member index = ExprQuotedVariable (VarIndex (compilerIdentifier packetName) (IndexSingle (ExprNumLiteral index)))
          capture = Stmt (builtin "set" [ExprVal (ExprLiteral packetName), ExprVal (ExprCommandSubst (Stmt (Command (compilerCommandName helperName) (map argumentExpression (Primitive.primitiveFields request))) NE.:| []))])
    ...
    lower origin candidate
      | Just count <- pureCost candidate,
        count >= 1 = do
          ...
          (after, result) <- operation origin (Primitive.BatchInteger frames)
    ...
    pureFrames origin = \case
      ...
      ArithmeticLiteral value -> pure ([], Primitive.PushInteger (ExprLiteral (show (wrapInteger value))))
      ArithmeticVariable name -> pure ([], Primitive.PushInteger (getter name))
      ArithmeticUnary operator inner -> do
        (before, frames) <- pureFrames origin inner
        pure (before, Primitive.ApplyUnary operator frames)
      ArithmeticBinary operator left right -> do
        ...
        pure (beforeLeft <> beforeRight, Primitive.ApplyBinary operator leftFrames rightFrames)
      value -> do
        (before, result) <- lower origin value
        pure (before, Primitive.PushInteger result)
    lowerNode origin = \case
      ...
      ArithmeticVariable name -> operation origin (Primitive.ReadInteger (getter name))
      ArithmeticUnary operator inner -> do
        (before, value) <- lower origin inner
        (after, result) <- operation origin (Primitive.UnaryInteger operator value)
        pure (before <> after, result)
      ...
      ArithmeticBinary operator left right -> do
        ...
        (after, result) <- operation origin (Primitive.BinaryInteger operator leftValue rightValue)
      ...
      ArithmeticUpdate position direction name -> do
        ...
        (after, newValue) <- operation origin (Primitive.BinaryInteger (if direction == Increment then Add else Subtract) oldValue (ExprLiteral "1"))
                  ...
                  Stmt
                    ( Pipeline
                        ( MkFishJobPipeline
                            ...
                            [PipeTo [] (Stmt (Command (compilerCommandName nativeHelper) [ExprVal (ExprLiteral "--abi"), ExprVal (ExprLiteral "2"), ExprVal (ExprLiteral (toText (cliOpcodeName CliInteger)))]))]
                        )
                    )
                    NE.:| []
              }
...
builtin :: Text -> [ExprOrRedirect] -> FishCommand Atomic TStatus
builtin name = Decorated DecBuiltin . Command (compilerCommandName name)
...
equal :: FishExpr TStr -> FishExpr TStr -> FishCommand Atomic TStatus
equal left right = builtin "test" [ExprVal left, ExprVal (ExprLiteral "="), ExprVal right]

notEqual :: FishExpr TStr -> FishExpr TStr -> FishCommand Atomic TStatus
...
conditional command yes no =
  let pipeline = MkFishJobPipeline False [] (Stmt command) []
