...
import Monk.Runtime.Abi2 (IntegerOpcode (..), integerOpcodeName)
...
unaryName = toText . integerOpcodeName . unaryOpcode

unaryOpcode :: UnaryOperator -> IntegerOpcode
unaryOpcode = \case
  UnaryPlus -> IntegerPos
  UnaryMinus -> IntegerNeg
  LogicalNot -> IntegerNot
  BitwiseNot -> IntegerInvert

binaryName :: BinaryOperator -> Text
binaryName = toText . integerOpcodeName . binaryOpcode

binaryOpcode :: BinaryOperator -> IntegerOpcode
binaryOpcode = \case
  Add -> IntegerAdd
  Subtract -> IntegerSub
  Multiply -> IntegerMul
  Divide -> IntegerDiv
  Remainder -> IntegerRem
  Power -> IntegerPow
  ShiftLeft -> IntegerShl
  ShiftRight -> IntegerShr
  LessThan -> IntegerLt
  LessEqual -> IntegerLe
  GreaterThan -> IntegerGt
  GreaterEqual -> IntegerGe
  Equal -> IntegerEq
  NotEqual -> IntegerNe
  BitAnd -> IntegerAnd
  BitXor -> IntegerXor
  BitOr -> IntegerOr
  LogicalAnd -> IntegerLogicalAnd
  LogicalOr -> IntegerLogicalOr
