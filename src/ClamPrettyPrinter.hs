...
prettyFishCommand = \case
  ...
  For var listArg body ->
    "for" <+> pretty var <+> "in" <+> (prettyFishArg listArg)
      <> hardline
...
prettyCaseItem (CaseItem pats body) =
  "case" <+> hsep (map prettyFishArg pats)
    <> hardline
    <> indent 2 (vsep (map prettyFishStatement body))
