...
request :: Text -> Text -> [ExprOrRedirect] -> FishStatement
request prefix operation arguments = Stmt (Command (requestHelperName prefix) (literal operation : arguments))

-- | Fields from the last successful read request. They stay private to the
-- evaluator and preserve empty strings through the NUL-framed reply.
responseValues :: Text -> FishExpr (TList TStr)
responseValues prefix = ExprVariable (VarIndex (prefix <> "session_fields") (IndexRange (Just (ExprNumLiteral 2)) Nothing))
...
requestDefinition prefix =
  ...
  where
    ...
    probe = Stmt (Command (NativeRuntime.runtimeHelperName prefix) (map literal ["--abi", "2", "descriptor-state"]))
    ...
    client = Stmt (Decorated DecCommand (CommandExpr (variable (NativeRuntime.runtimePathName prefix)) (map literal ["--abi", "2", "session-client", "--reply"])))
    pipeline = Stmt (Pipeline (MkFishJobPipeline False [] input [PipeTo [] client] False))
    arm bits = MkCaseItem (ExprLiteral (show bits) :| []) (Stmt (Begin (pipeline :| [choose (builtin "test" [ExprVal (member "pipestatus" (-1)), literal "=", literal "0"]) [] failure]) [RedirectVal (MkRedirect (RedirectFD (fd + 3)) (if fd == 0 then RedirectIn else RedirectOut) (RedirectTargetFD fd)) | fd <- [0 .. 2], testBit bits fd]) :| [])
    capture = ExprCommandSubst (builtin "string" [literal "split0", RedirectVal (MkRedirect RedirectStdin RedirectIn (RedirectFile (variable "MONK_SESSION_REPLY")))] :| [])
    body =
      [ probe,
        ...
        Stmt (Decorated DecBuiltin (Set [SetLocal] packet capture)),
        choose (builtin "test" [ExprVal (ExprQuotedCommandSubst (builtin "count" [ExprVal (ExprVariable (VarAll packet))] :| [])), literal "-ge", literal "2"]) [] failure,
        choose
          (builtin "test" [ExprVal (member "argv" 1), literal "=", literal "read"])
          [Stmt (Decorated DecBuiltin (Set [SetGlobal] (prefix <> "session_fields") (ExprVariable (VarIndex packet (IndexRange (Just (ExprNumLiteral 3)) Nothing)))))]
          [ choose
              (builtin "test" [ExprVal (member "argv" 1), literal "=", literal "substitution"])
              [ choose (builtin "test" [ExprVal (ExprQuotedCommandSubst (builtin "count" [ExprVal (ExprVariable (VarAll packet))] :| [])), literal "=", literal "5"]) [] failure,
                Stmt (Decorated DecBuiltin (Set [SetGlobal] (prefix <> "session_endpoint") (ExprVariable (VarIndex packet (IndexRange (Just (ExprNumLiteral 4)) Nothing)))))
              ]
              [choose (builtin "test" [ExprVal (ExprQuotedCommandSubst (builtin "count" [ExprVal (ExprVariable (VarAll packet))] :| [])), literal "=", literal "3"]) [] failure]
          ],
        ...
        builtin "return" [ExprVal (member packet 2)]
      ]
...
launchSession prefix body =
  [ set [SetLocal] "fish_read_limit" (ExprLiteral "0"),
    Stmt (Decorated DecBuiltin (Set [SetLocal] capsule captured)),
    choose (builtin "test" [ExprVal (ExprQuotedCommandSubst (builtin "count" [ExprVal (ExprVariable (VarAll capsule))] :| [])), literal "=", literal "4"]) [] failure,
    choose (builtin "test" [ExprVal (member capsule 1), literal "=", literal "ok"]) [] failure,
    Stmt (Decorated DecExec (CommandExpr (variable path) (map literal ["--abi", "2", "session-run", "--capsule"] <> [ExprVal (member capsule 3), ExprVal (member capsule 4), ExprVal (variable path), ExprVal (ExprVariable (VarAll "argv"))])))
  ]
  where
    ...
    prepare = Stmt (Decorated DecCommand (CommandExpr (variable path) (map literal ["--abi", "2", "session-prepare"])))
    captured = ExprCommandSubst (Stmt (Pipeline (MkFishJobPipeline False [] writer [PipeTo [] prepare, PipeTo [] (builtin "string" [literal "split0"])] False)) :| [])

failure :: [FishStatement]
failure = [builtin "printf" [literal "%s\n", literal "monk.runtime: session control failed", RedirectVal (MkRedirect RedirectStdout RedirectOut (RedirectTargetFD 2))], builtin "exit" [literal "125"]]
